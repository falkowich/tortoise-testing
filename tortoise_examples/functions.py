...
async def run():
    ...
    print(
        await Tournament.all()
        .annotate(
            events_count_with_filter=Count("events", _filter=Q(name="New Tournament"))
        )
    ...
    print(
        await Tournament.annotate(clean_desciption=Coalesce("desc", "")).filter(
            clean_desciption=""
        )
    )

    print(
        await Tournament.annotate(trimmed_name=Trim("name")).filter(
            trimmed_name="final tournament"
        )
    )
    ...
    print(
        await Tournament.annotate(name_lo=Lower("name")).filter(
            name_lo="new tournament"
        )
    )
    print(
        await Tournament.annotate(name_lo=Upper("name")).filter(
            name_lo="NEW TOURNAMENT"
        )
    )
